...
public class ResourceClaimController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(UserAndRoleTuple), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<ActionResult> GetUsersAndRolesResourceClaims(
        ...
        [FromRoute] string resourceIdentifier)
    {
        var users = await _apiDbContext.Users
                                       .Include(
                                            (e) => e.Claims!.Where(
                                                (claim) => claim.Identifier.StartsWith(resourcePrefix)
                                                        && claim.Value == resourceIdentifier
                                            )
                                        )
                                       .AsNoTracking()
                                       .Where(
        ...
        [FromRoute] string resourceClaim)
    {
        ...
        var user = await _apiDbContext.Users
                                      .Include(
                                           (e) => e.Claims!.Where(
                                               (claim) => claim.Identifier == resourceClaim
                                                       && claim.Value == resourceIdentifier
                                           )
                                       )
                                      .AsNoTracking()
    ...
    // Create a method that removes a claim by its PrimaryKey

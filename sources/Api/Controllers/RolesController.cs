...
public class RolesController : ControllerBase
{
    ...
    public async Task<IEnumerable<PlainRoleDto>> GetAllRolesAsync(CancellationToken cancellationToken)
    {
        if (!User.TryGetUserId(out var currentUserId))
            throw new UnauthorizedAccessException();
        var roles = User.HasClaim(Claims.Administrative.All, string.Empty)
            ? await _apiDbContext.Roles
                .Select(role => new PlainRoleDto
                    {
            ...
            : await _apiDbContext.Users
                .Where(user => user.PrimaryKey == currentUserId)
    ...
    public async Task<IActionResult> SetUserRoleToAsync(
    ...
    )
    {
        ...
        var existingRole = await _apiDbContext.Roles.SingleAsync(q => q.PrimaryKey == roleId, cancellationToken);
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    public async Task<IActionResult> SetRoleClaimToAsync(
        [FromRoute] long roleId,
    ...
    )
    {
        if (!User.TryGetUserId(out var currentUserId))
            return Unauthorized();
        await using var dbTransaction = await _apiDbContext.Database.BeginTransactionAsync(cancellationToken);
        var existingClaim = await _apiDbContext.Claims.SingleAsync(q => q.PrimaryKey == roleId, cancellationToken);
        var isInRole = User.HasClaim(Claims.Administrative.Role, string.Empty)
                       || await _apiDbContext.Users
                           .Where(q => q.PrimaryKey == currentUserId)
                           .SelectMany(q => q.Roles!)
    ...
    )
    {
        ...
        await _apiDbContext.Roles.AddAsync(role, cancellationToken);
        await _apiDbContext.SaveChangesAsync(cancellationToken);
        _logger.LogInformation(
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    public async Task<IActionResult> DeleteRoleAsync([FromRoute] long roleId, CancellationToken cancellationToken)
    {
        if (!User.TryGetUserId(out var currentUserId))
            return Unauthorized();
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [Authorize(Claims.Administrative.Role)]
    public async Task<IActionResult> GetClaimsOfRole([FromRoute] long roleId, CancellationToken cancellationToken)
    {
        ...
        if (role is null)
            return NotFound();
        return Ok(role.Claims!.Select(e => e.ToPlainDto()).ToArray());
    }
}
